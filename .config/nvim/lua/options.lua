...
opt.tabstop = indent     -- insert 2 spaces for a tab (or not 2 idk)

-- search
opt.incsearch = true  -- real time search
...
opt.number = true         -- set relative numbered lines
opt.scrolloff = 18        -- minimal number of screen lines to keep above and below the cursor
opt.sidescrolloff = 2     -- minimal number of screen columns to keep to the left and right (horizontal) of the cursor if wrap is `false`
...
opt.history = 500    -- keep 500 lines of history
opt.redrawtime = 1500
opt.timeoutlen = 250 -- time to wait for a mapped sequence to complete (in milliseconds)
opt.ttimeoutlen = 10
opt.updatetime = 50  -- signify default updatetime 4000ms is not good for async update
...
vim.diagnostic.config({
    virtual_text = {
        prefix = "", -- or ">>" or "" if you use Nerd Font
    },
    signs = true, -- keep the E and W in the sign column
...
-- Show diagnostics when you hover over a line
